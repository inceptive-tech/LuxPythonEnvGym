...
class AgentPolicy(AgentWithModel):
    ...
    def get_reward(self, game, is_game_finished, is_new_turn, is_game_error):
                ...
                if city_id in self.city_fuel_last:
                    ...
                    if fuel_diff > 0:
                        if city_fuel < fuel_required_next_night:
                            potential["rew/r_city_fuel"] += unit_reward * len(city.city_cells) / 100
                        elif city_fuel < fuel_required_next_night + fuel_required_full_night:
                            potential["rew/r_city_fuel"] += 0.5 * unit_reward * len(city.city_cells) / 100
                        elif city_fuel < fuel_required_next_night + 2 * fuel_required_full_night:
                            potential["rew/r_city_fuel"] += 0.1 * unit_reward * len(city.city_cells) / 100
        ...
        potential["rew/r_units_created"] = max(unit_count - self.units_last, 0) * unit_reward
        potential["rew/r_units_lost"] = min(unit_count - self.units_last, 0) * unit_reward * 1.25
        self.units_last = unit_count
        # reward for research
        potential["rew/r_research"] = (researchPoints - self.researchPoints_last) * unit_reward * (0 if researchPoints > 200 else (0.5 if researchPoints > 50 else 0.75))
        self.researchPoints_last = researchPoints


        # Give a reward for city creation/death. 0.1 reward per city.
        potential["rew/r_city_tiles_created"] = max(city_tile_count - self.city_tiles_last, 0) * unit_reward * 2
        potential["rew/r_city_tiles_lost"] = min(city_tile_count - self.city_tiles_last, 0) * unit_reward * 2 * 1.5
        ...
        potential["rew/r_city_tiles_end"] = 0
        potential["rew/r_clustered_resources"] = self.resource_clusterer.getReward(game, self.team, unit_reward)
        reward = 0
        if is_game_finished:
            self.is_last_turn = True
            potential["rew/r_city_tiles_end"] = city_tile_count
            if city_tile_count is 0:
                potential["rew/r_city_tiles_end"] = -2
            ...
            if win_team == self.team:
                reward = 1000 * unit_reward + city_tile_count[self.team]  # Win
            elif win_team is None:
                reward = -10 * unit_reward  # Tie
            else:
                reward = -1000 * unit_reward  # Loss
            '''
